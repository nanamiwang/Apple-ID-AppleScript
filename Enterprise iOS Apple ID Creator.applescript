...
--Used to store the file location of the iTunes U "App Page Shortcut". Updated dynamically on run to reference a child folder of the .app bundle (Yes, I know this isn't kosher)
-- AF 2012-05-14 Open location instead of .inetloc
property itunesuLinkLocation : "itms://itunes.apple.com/us/app/itunes-u/id490217893?mt=8"

--Master delay timer (in seconds) for slowing the script down at specified sections. Usefull for tweaking the entire script's speed
...
property supportedItunesVersions : {"12.4.3"}
...
property supportedFreeButtonDescriptions : {"$0.00 Get, iTunes U", "0,00 € Get, iTunes U"}
		...
		if scriptAction is "Continue" then
			...
			if scriptAction is not "Abort" then
				...
				repeat with loopCounter from 1 to userCount
					...
					installItunesU() ---------------------------------------------------------------------------------------------------------------------------------------------------------------------Go to the iTunes U App page location to kick off Apple ID creation with no payment information
					
					delay 1 --Fix so iTunes is properly tested for, instead of just manually delaying
					
					repeat
						set lcdStatus to GetItunesStatusUntillLcd("Does Not Match", itunesAccessingString, 4, "times. Check for:", 120, "intervals of", 0.25, "seconds") ------------------------Wait for iTunes to open (if closed) and the iTunes U page to load
...
on verifyPage(expectedElementString, expectedElementLocation, expectedElementCount, verificationTimeout, requiresGroup)
	--Element count feature of this handler appears disabled
	tell application "System Events"
		...
		-- set UI location of HTML content container
		set htmlContentUiElement to UI element 1 of scroll area 1 of group 1 of group 1 of splitter group 1 of window "iTunes" of application process "iTunes"
		...
		(*
		--The element count feature seems disabled in this handler by code in the repeat loop 
		--  so the set statement for counting elements has been removed as it breaks easily due to iTunes changes
		--set elementCount to count every UI element of UI element 1 of scroll area 1 of splitter group 1 of window 1 of application process "iTunes" --original line that caused script error for current iTunes
		set elementCount to count every UI element of UI element 1 of scroll area 1 of group 1 of group 1 of splitter group 1 of window 1 of application process "iTunes"
		*)
		...
		repeat with timeoutLoopCounter from 1 to verificationTimeout --Loop will be ended before reaching verificationTimeout if the expectedElementString is successfully located
			...
			--line below seems to defeat the purpose of the element count by setting expected and actual counts equal
			--if expectedElementCount is not elementCount then set expectedElementCount to elementCount
			...
			*)
			if true then
				...
				if requiresGroup then
					set elementToTest to UI element expectedElementLocation of group 2 of htmlContentUiElement
				else
					set elementToTest to UI element expectedElementLocation of htmlContentUiElement
				end if
...
on SignOutItunesAccount()
	if scriptAction is "Continue" then --This is to make sure an abort hasn't been thrown
		tell application "System Events"
			--Tell iTunes to open iTunes U. Still submits information to Apple but moves the script along much faster
			tell application "iTunes" to open location itunesuLinkLocation
			delay masterDelay
...
on GetItunesStatusUntillLcd(matchType, stringToMatch, matchDuration, "times. Check for:", checkDuration, "intervals of", checkFrequency, "seconds")
	local loopCounter, matchedFor, itunesLcdText
	...
	repeat
		...
		tell application "System Events"
			try
				set item loopCounter of itunesLcdText to value of static text 1 of scroll area 1 of window "iTunes" of application process "iTunes"
			on error
				--scroll area one has no children and no value property when only Apple Logo is displayed so:
				set item loopCounter of itunesLcdText to description of scroll area 1 of window "iTunes" of application process "iTunes"
			end try
		end tell
...
on installItunesU()
	...
	if scriptAction is "Continue" then --This is to make sure an abort hasn't been thrown
		...
		tell application "iTunes" to open location itunesuLinkLocation
		delay (masterDelay * processDelay)
		set pageVerification to verifyPage("iTunes U", "iTunes U", 42, netDelay, true) --Looking for "iTunes U" on a page with a count of 42 elements, with a timeout of netDelay, and it requires the use of "group 1" for checking
		
		if pageVerification is "verified" then --Actually click the button to obtain iTunes U
			delay (masterDelay * processDelay)
			tell application "System Events"
				try
					set freeButton to button 1 of group 3 of htmlContentUiElement
			...
			set errorList to errorList & "Unable to verify that iTunes is open at the iTunes U App Store Page."
		end if
	...
	if scriptAction is "Continue" then --This is to make sure an abort hasn't been thrown
		...
		tell application "System Events"
			if value of static text 4 of window 1 of application process "iTunes" is "Sign in to the iTunes Store" then
				try
					click button "Create New Apple ID" of window 1 of application process "iTunes"
				on error
					set errorList to errorList & "Unable to locate and click button ''Create New Apple ID'' on ID sign-in window"
				end try
			else
				set errorList to errorList & "Unable to locate sign-in window and click ''Create New Apple ID''"
			end if
		end tell
	end if
end ClickCreateNewAppleIDButton

-----------------------------------------

on ClickContinueOnPageOne()
	delay (masterDelay * processDelay) + 4
	...
	if pageVerification is "verified" then
		...
		try
			tell application "System Events"
				set htmlContentUiElement to UI element 1 of scroll area 1 of group 1 of group 1 of splitter group 1 of window "iTunes" of application process "iTunes"
				set contButton to button "Continue" of htmlContentUiElement
...
on AgreeToTerms()
	...
	if pageVerification is "verified" then
		tell application "System Events"
			...
			try
				--set agreeCheckbox to checkbox 1 of group 4 of group 2 of UI element 1 of scroll area 1 of splitter group 1 of window 1 of application process "iTunes"
				set agreeCheckbox to checkbox 1 of group 4 of group 2 of htmlContentUiElement
			...
			if scriptAction is "Continue" then
				try
					...
					set agreeButton to button "Agree" of htmlContentUiElement
	...
	tell application "System Events"
		...
		try
			...
			repeat with loopCounter from 1 to maximum
				...
				set focused of theField to true
				delay masterDelay
				keystroke " " -- Spacebar to open the menu
				delay masterDelay
...
on ProvideAppleIdDetails(appleIdEmail, appleIdPassword, appleIdSecretQuestion1, appleIdSecretAnswer1, appleIdSecretQuestion2, appleIdSecretAnswer2, appleIdSecretQuestion3, appleIdSecretAnswer3, rescueEmail, userBirthMonth, userBirthDay, userBirthYear)
	if scriptAction is "Continue" then --This is to make sure an abort hasn't been thrown
		...
		if pageVerification is "Verified" then
			tell application "System Events"
				set theForm to group 2 of htmlContentUiElement
				-----------
				...
				-----------
				tell me to FillInKeystroke("Retype your password", text field 1 of group 4 of group 3 of theForm, appleIdPassword)
				-----------
				tell me to FillInPopup("First Security Question", pop up button 1 of group 1 of group 6 of theForm, appleIdSecretQuestion1, 6)
				tell me to FillInField("First Answer", text field 1 of group 2 of group 6 of theForm, appleIdSecretAnswer1)
				-----------
				tell me to FillInPopup("Second Security Question", pop up button 1 of group 1 of group 7 of theForm, appleIdSecretQuestion2, 6)
				tell me to FillInField("Second Answer", text field 1 of group 2 of group 7 of theForm, appleIdSecretAnswer2)
				-----------
				tell me to FillInPopup("Third Security Question", pop up button 1 of group 1 of group 8 of theForm, appleIdSecretQuestion3, 6)
				tell me to FillInField("Third Answer", text field 1 of group 2 of group 8 of theForm, appleIdSecretAnswer3)
				-----------
				tell me to FillInField("Optional Rescue Email", text field 1 of group 11 of theForm, rescueEmail)
				-----------
				--start localization
				...
				-----------
				set releaseCheckbox to checkbox 1 of group 15 of theForm
				set newsCheckbox to checkbox 1 of group 16 of theForm
				if value of releaseCheckbox is 1 then
					tell me to ClickThis("Recommendations and information about new releases for music, apps, movies, TV, books, and podcasts.", releaseCheckbox)
				end if
				if value of newsCheckbox is 1 then
					tell me to ClickThis("Announcements and recommendations about Apple products, services, and updates.", newsCheckbox)
				end if
				-----------
				...
				if scriptAction is "Continue" then
					click button 3 of htmlContentUiElement
				end if
			end tell
		...
		tell application "System Events"
			...
			if dryRun is false then --Click the "Create Apple ID" button as long as we aren't in "Dry Run" mode
				if scriptAction is "Continue" then --Continue as long as no errors occurred
					try
						click button "Create Apple ID" of htmlContentUiElement
					on error
